...
public interface MessageBus {
     ...
     * @pre !mb.hasQueue(m) before register
     * @post mb.hasEvent(m,event)==true after register
     */
     ...
     * @param m    	The subscribing micro-service.
     * @pre !mb.hasQueue(m) before register
     * @post mb.hasBroadcast(m,broad)==true after register
     */
     ...
     * @post: mb.hasBroadcast(m,broad)==true
     */
     ...
     * @pre: mb.sendEvent(event)==null before register
     * @post: mb.hasEvent(m,event)==true
     */
     ...
     * @pre getQueue==null
     * @post getMicroService==m
     * @inv
     * @pre: mb.hasQueue(m)==false
     * @post: mb.hasQueue(m)==true
     */
     ...
     *                              to became available.
     *
     * @pre: check IllegalStateException is thrown before register
     * @inv: check InterruptedException is thrown if interrupted while waiting
     * @post: mb.hasEvent(m, event)==false
     */
    Message awaitMessage(MicroService m) throws InterruptedException;

    /**
     *
     * @param m the micro-service we want to get it's designated queue
     * @param <T>
     * @return returns the designated queue
     */
    boolean hasQueue(MicroService m);

    <T>  boolean hasEvent(MicroService m, Event<T> event);

    boolean hasBroadcast(MicroService m, Broadcast broad);
